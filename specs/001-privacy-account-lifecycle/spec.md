...
- Owner decision: Approve the requirement that a restore cannot revive a deleted account and must fail closed when deletion reconciliation cannot be verified. The proposed independent ledger protocol and retention guarantees are not approved pending provider-capability evidence, failure-handling proof and an isolated restore exercise. The numerical retention limits in this draft remain proposed requirements.
