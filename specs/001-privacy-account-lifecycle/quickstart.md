...
4. In an isolated Neon test project, note time `T`, then delete A. With API ingress disabled, restore to `T` with `--preserve-under-name`. Diff PrivacyAccountId against the preserved branch, re-delete A and verify zero A records while B remains intact. Rotate signing credentials, delete the preserved branch and only then demonstrate controlled reopening.
