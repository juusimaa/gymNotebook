...
- Reviewed plan/schema/API/UI, reviewed migration, synthetic accounts A and B, and an isolated Neon test branch or project for restore exercises. Never perform deletion/restore exercises against a real user's data.
...
- No independent evidence store is configured in the chosen design (research R6). The future implementation must add any new configuration names/setup to README.md and `.env.example` without committing values.
...
2. Open A in two sessions and keep B active. Delete A using correct password/confirmation. Success follows active commit and the `deletion.committed` log line. Assert zero A account/exercise/workout/block/set/acknowledgement rows and no export files. Only the minimal deletion log lines and reviewed logs remain; B is byte/value-equivalent to its baseline.
...
4. Inject a failure before deletion commit: all notebook data survives; no success or partial usable notebook. Inject an ambiguous commit: safe uncertain outcome, no false rollback/success. Verify intent, committed and rolled-back log lines appear exactly for the matching outcomes.
...
7. Delete the reference 100,000-set account within 60 seconds under recorded normal conditions. Record wait and commit durations separately to expose contention or provider latency.

## 4. Controlled concurrency and transport

Run two independent application hosts against the same real Postgres. For every scenario test both orderings with barriers before authorization, lock acquisition, commit and response flush:
...
2. Use controlled clocks in tests for 24-hour export limits (if any persistent copy is introduced), 30-day backup/log and 31-day deletion-evidence boundaries. Test a log collected 20 days before deletion: at most 10 days remain. Non-justified logs are erased/anonymized on deletion. Copying or restoring cannot move deadlines.
3. Verify strict Azure retention setting, table overrides/total retention, ingestion delay, extra sinks and existing-data transition. Verify every Neon branch/restore window/snapshot and manual/non-production copy. Re-run this check after any change to Neon plan, history retention, snapshots or branches (research R6). Retention automation needs observed disposal evidence, not just a configured timer.
4. In an isolated Neon test project, note time `T`, then delete A. With API ingress disabled, restore to `T` with `--preserve-under-name`. Diff PrivacyAccountId against the preserved branch, re-delete A and verify zero A records while B remains intact. Before deleting A, change B's password; verify the restore copies B's newer PasswordHash and TokenVersion from the preserved branch, so the old password fails. Register a new account after `T` in a separate run and verify its old token fails after rotation. Rotate signing credentials, delete the preserved branch and only then demonstrate controlled reopening.
5. Repeat for the fallback:
   - Treat the preserved branch as unusable and reconcile from log lines: A is re-deleted.
   - Simulate a log ingestion gap: access stays closed.
   - Leave an intent line with no committed or rolled-back line: that account is suspended, not erased or revived, and cannot sign in.
   - Leave an intent with a rolled-back line: that account is untouched.
   - Interrupt before and after each step, and replay twice safely.
