...
- **Log Analytics retention:**
  ...
  - **T069 (2026-09-26):** a re-inspection found 14 tables at 90/90: `AzureActivity`, `Usage` and twelve App\* tables, including `AppGenAIContent`, which the list above did not name. `infra/modules/log-analytics.bicep` now pins all 14 to 30/30 on the Analytics plan and sets `immediatePurgeDataOn30Days: true`, stating `enableLogAccessUsingOnlyResourcePermissions: true` so the live value is kept. Module-only what-if against prod showed the 14 retention changes and nothing outside Log Analytics. The workspace `features` showed as "NoEffect" (what-if cannot read it), and `schema`/`protectionLevel` showed as "Delete" (Azure-managed on built-in tables). **Pending live verification after deployment:** no table other than 30/30, `immediatePurgeDataOn30Days` present in the workspace features, and built-in table schemas intact. Microsoft documents the purge flag at workspace level only, so whether it also covers per-table 30-day overrides stays unverified unless Azure's documentation or observed row ages (T077) show it.
