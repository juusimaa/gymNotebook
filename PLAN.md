...
- **Responses carry what the screens draw, so a list is one request.** Checking the UI spec against the response records (after milestone 4) found four things a screen showed that nothing returned, and each became a field rather than a client-side join: a `GET /workouts` row carries `exerciseNames` (position order), `exerciseCount`, `setCount` and `endedAt`, so the sessions list never fetches a page to render it; each block in `GET /workouts/{id}` carries its exercise's `isBodyweight`, which is what the session page branches on for "best reps" versus e1RM; each `GET /exercises` result carries `lastSet` — the last working set of the most recent session that logged it, `null` if none — for the autocomplete hint; and `GET /auth/me` returns the `username` for the cover, looked up from the row rather than minted into the token so a token can't outlive a rename. All are computed in the same query as the row they hang off, never per row.
...
- **UI design — proposed, see [`docs/ui/`](docs/ui/README.md).** The spec and prototype settle the visual direction, the tokens and the component-library question (none) for the screens in milestones 5, 7, 8 and 9. The implemented screens include their own loading, empty and error states; the remaining milestone 9 exercise-management states must stay aligned with the prototype when they are built.
