...
11. **Privacy and account lifecycle** — in progress, through Spec Kit ([specs/001-privacy-account-lifecycle/](specs/001-privacy-account-lifecycle/)), behind the fail-closed `PRIVACY_LIFECYCLE_ENABLED` flag. So far: self-hosted fonts, the nginx access log turned off, the flag itself, spike Part A of the locking design, and **the lifecycle foundation** (tasks T010–T027), which ships unflagged because it changes existing routes. That foundation is the `User` privacy columns and their migration, the account-lifecycle locking described under Auth → Account lifecycle coordination, change-password's move to exclusive access, and login's suspension check. On the frontend, `ApiError` now carries the optional `code` from the few error bodies that have one, so login's `account_suspended` 403 reads differently from register's invite-code 403. The tests run two app hosts against one Testcontainers Postgres (`TwoHostGymNotebookFixture`) and synchronize through `pg_locks` and an EF commit interceptor rather than sleeps. The streaming test runs on real Kestrel, because `TestServer` has no socket buffers to fill. An unchanged API process had nothing to block on, so every existing test passed with the filter attached; the overhead measured in the spike was about 1–3 ms per request locally.
    ...
    **User story 3, the notebook export** (tasks T044–T055), is implemented behind the flag. Its release gate, spike Part B on a disposable Azure environment (T053), is still open:
    ...
    - **The screen saves only a complete file.** `/account/export` (`ExportData.tsx`) reads the whole body and checks it parses as JSON before saving it, as a second defence should a proxy ever turn an abort into a clean end of stream. The password is cleared once sent and never kept. The file sits in a temporary object URL (`api/download.ts`) that is revoked a few seconds after the save starts, or at once when the screen is left. To read the raw body, `api/client.ts` now exposes `send()`, and `request()` is built on it.

    **User story 4, account deletion** (tasks T056–T068), is implemented behind the flag:
    - **One transaction under exclusive access.** `POST /account/delete` (`AccountDeletion.cs`) requires the current password and `confirmDeletion: true`. It verifies the password before any lock (BCrypt stays outside it; a password change in between shows up under the lock as a token-version mismatch), then takes the account's lifecycle lock exclusively, waiting up to 15 s for in-flight requests. It deletes workouts (the database cascades blocks and sets), then exercises, then the `User` row, and commits. Requests queued behind it re-check after the commit and get 401, so every session ends with the account. Once the lock is held the work runs without the request's cancellation token: closing the page doesn't decide whether a confirmed deletion happens. A `statement_timeout` set with `set_config(…, true)` bounds each statement instead.
    - **Three log lines are the restore fallback's evidence** (research R6): `deletion.intent` before the deletes, `deletion.committed` after the commit, `deletion.rolled_back` when it definitely didn't commit. They carry only the event, `PrivacyAccountId` and the deletion boundary. A failure before `COMMIT` is sent is a definite rollback, and so is an error the database returns for the `COMMIT`: both give 503 `temporarily_unavailable`. A connection failure *during* the commit can't tell, so the response is 503 `deletion_outcome_unknown` and only the intent line exists, which is how the restore procedure recognizes an unknown outcome. `appsettings.json` pins the category to `Information`, so a stricter default log level can't silently drop the evidence.
    - **The response carries only dates.** The deadlines are counted from the boundary, taken just before the intent line, so a slow commit can't move them later: backups expire by boundary + 30 days, and the deletion log lines by + 31 days. They use whole days in UTC, not months, which a controlled-clock test checks across month ends and a leap day. No token and nothing about the account is returned. A retry after a lost response gets 401, never a second success.
    - `AccountDeletion.cs` is the only file the Q2d source scan allows to remove users. The concurrency tests run the real endpoint on a second host against every notebook write, password change, acknowledgement, `/auth/me`, reads and the export, in both orders. Locally the 100,000-set reference notebook deletes in well under a second.
    - **Two screens, and one claim only when it's true.** `/account/delete` (`DeleteAccount.tsx`) has a review step (what goes, the optional export, what remains and for how long) and a separate password step. Its failures sort into two groups (`deletionOutcome.ts`). Wrong password, too many attempts and 503 `temporarily_unavailable` keep the form, because nothing was deleted. An unconfirmed commit, a broken connection or a 401 replace it with copy that claims neither outcome. The public `/account/deleted` renders the deletion only from the router state handed over after the server's success, so the address alone shows a neutral signed-out screen.
    - **Session invalidation is now app-wide** (`auth/invalidation.ts`), replacing milestone 5's "the route guard is the one place a 401 is handled". Any 401 on a request that carried a token ends the session: token, in-flight requests (the client now tracks them, bodies included, so a sign-out cuts an export download too), export object URLs, and every notebook screen and draft, dropped by navigating to `/login`. Change-password (whose wrong-password answer is also 401) and deletion opt out with `unauthorized: 'local'`. Other tabs follow through the token key's `storage` event, and a back/forward-cache restore reloads so the guards run first. A 401 on a write adds the Q5 warning on sign-in, since the change may already be saved.
